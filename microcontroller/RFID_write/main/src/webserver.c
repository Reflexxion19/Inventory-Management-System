...
static esp_err_t post_handler(httpd_req_t *req)
{
    ...
    switch(pair_count) {
        case 2:
            if(strcmp(pairs[0].value, "auth_message") == 0){
                ...
                char *processed_data = malloc(1024);
                if (processed_data) {
                    snprintf(processed_data, 1024, "{\"device_name\":\"mk1\",\"type\":\"auth_response\",\"message\":\"%s\"}", url_encoded);
                ...
                xTaskCreate(storage_open, "storage open", 1024, NULL, 0, NULL);
            }
            break;
    ...
    const char resp[] = "Data processed successfully";
